...
import {
    ...
    Screens,
    useAsyncNavigation
} from './AsyncNavigationWithoutTypes';
import { now, Styles } from '../common/Utilities';
import { differenceInDays, format, isAfter, isBefore } from 'date-fns';
import DatePicker from 'react-native-date-picker';
import { AsyncNavigationContainer } from './AsyncNavigation';

const AsyncNavigationStack = createNativeStackNavigator();

export const AsyncNavigationAppWithoutTypes: React.FC = () => (
            ...
            renderItem={info => {
                ...
                return (
                    <TouchableHighlight
                        ...
                        activeOpacity={0.5}
                        onPress={async () => {
                            const { result } = await navigator.push<any, InvoiceDetailOutput>(
...
type InvoiceDetailOutput = Invoice;
const InvoiceDetailScreen = () => {
    ...
    const onPayPressed = async () => {
        const { result: authenticationResult } = await navigator.navigate<
            any,
            AuthenticationScreenOutput
        >(Screens.Authentication, {});
        if (!authenticationResult?.successful) {
            return;
        }
        const { result: card } = await navigator.push<any, SelectCardOutput>(
            Screens.SelectCard,
            {}
        );
        if (!card) {
        ...
        if (currency !== card.currency) {
            const { result: selectedCurrency } = await navigator.push<any, SelectCurrencyOutput>(
        ...
        if (isBefore(paymentDate, invoice.dueDate)) {
            ...
            const { result: selectedDate } = await navigator.push<
                SelectDateInput,
                SelectDateOutput
            >(Screens.SelectDate, {
...
Date: ${format(paymentDate, 'dd/MM/yyyy')}`
        ...
        invoice.paid = true;
        await navigator.completeAndGoBack<InvoiceDetailOutput>(invoice);
    };
    return (
...
type AuthenticationScreenOutput = { successful: boolean };
    ...
    return (
        <Column style={{ padding: 16, borderRadius: 8, backgroundColor: 'white' }}>
            ...
            <Button
                title="Successful"
                onPress={() =>
                    navigator.completeAndGoBack<AuthenticationScreenOutput>({ successful: true })
                }
            />
            <Button
                title="Not successful"
                onPress={() =>
                    navigator.completeAndGoBack<AuthenticationScreenOutput>({ successful: false })
                }
            />
            ...
            renderItem={info => {
                ...
                return (
                    <TouchableHighlight
                        underlayColor="rgba(255, 255, 255, 0.0)"
                        activeOpacity={0.5}
                        onPress={() => navigator.completeAndGoBack<SelectCardOutput>(card)}>
                        <Column style={{ padding: 16, borderRadius: 8, backgroundColor: 'white' }}>
                            <Row>
...
type SelectCurrencyOutput = Currency;
            ...
            renderItem={info => {
                ...
                return (
                    <TouchableHighlight
                        underlayColor="rgba(255, 255, 255, 0.0)"
                        activeOpacity={0.5}
                        onPress={() => navigator.completeAndGoBack<SelectCurrencyOutput>(currency)}>
                        <Column style={{ padding: 16, borderRadius: 8, backgroundColor: 'white' }}>
                            <Text>{currency}</Text>
                        </Column>
                    </TouchableHighlight>
...
type SelectDateInput = {
    defaultSelection?: number;
    maxDate?: number;
    minDate?: number;
};
type SelectDateOutput = number;
const SelectDateScreen = () => {
    const { navigator, params } = useAsyncNavigation<SelectDateInput>(Screens.SelectDate, {
        headerRight: () => <Button title="Close" onPress={() => navigator.goBack()} />
    });
    ...
    return (
        <Column style={{ alignItems: 'center' }}>
            ...
            <Button
                title="Select"
                onPress={() => navigator.completeAndGoBack<SelectDateOutput>(date.getTime())}
            />
        </Column>
    );
};
